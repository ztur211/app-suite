/**
 * Integration-test globalSetup for api-eat:
 *
 *   1. Provisions things_eat + things_auth via @things/testing's testcontainer
 *      helper, including the read-only auth_reader role for cross-app session
 *      lookup.
 *   2. Pushes api-eat's domain schema into things_eat.
...
declare global {
  var __THINGS_EAT_PG__: SuitePostgres | undefined;
  var __THINGS_AUTH_OWNER_URL__: string | undefined;
}

export default async function globalSetup(): Promise<void> {
  const suite = await setupSuitePostgres({ apps: ['auth', 'eat'], authReader: true });

  execSync('npx prisma db push --skip-generate --schema=prisma/schema.prisma', {
    cwd: path.resolve(__dirname),
    stdio: 'inherit',
    env: { ...process.env, DATABASE_URL: suite.urls.eat },
  });
  ...
  process.env['DATABASE_URL'] = suite.urls.eat;
  ...
  globalThis.__THINGS_EAT_PG__ = suite;
