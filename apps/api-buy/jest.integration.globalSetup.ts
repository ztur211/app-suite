/**
 * Integration-test globalSetup for api-buy:
 *
 *   1. Provisions things_buy + things_auth via @things/testing's testcontainer
 *      helper, including the read-only auth_reader role for cross-app session
 *      lookup.
 *   2. Pushes api-buy's domain schema into things_buy.
...
declare global {
  var __THINGS_BUY_PG__: SuitePostgres | undefined;
  var __THINGS_AUTH_OWNER_URL__: string | undefined;
}

export default async function globalSetup(): Promise<void> {
  const suite = await setupSuitePostgres({ apps: ['auth', 'buy'], authReader: true });

  execSync('npx prisma db push --skip-generate --schema=prisma/schema.prisma', {
    cwd: path.resolve(__dirname),
    stdio: 'inherit',
    env: { ...process.env, DATABASE_URL: suite.urls.buy },
  });
  ...
  process.env['DATABASE_URL'] = suite.urls.buy;
  ...
  globalThis.__THINGS_BUY_PG__ = suite;
